#!/usr/bin/env python3
"""
Thinning Recommendation System
...
class ThinningRecommender:
    ...
    def create_feature_row(self, part_data, reduction_pct):
        ...
        # Mass features (if available)
        if 'orig_mass_kg' in part_data and pd.notna(part_data['orig_mass_kg']):
            features['mass_reduction_pct'] = reduction_pct  # Assume same as element reduction
            features['orig_mass_kg'] = part_data['orig_mass_kg']
        else:
            features['mass_reduction_pct'] = reduction_pct
            features['orig_mass_kg'] = 1.0  # Default value
        
        # Volume features (if available)
        if 'orig_volume_m3' in part_data and pd.notna(part_data['orig_volume_m3']):
            features['volume_reduction_pct'] = reduction_pct  # Assume same as element reduction
            features['orig_volume_m3'] = part_data['orig_volume_m3']
        else:
            features['volume_reduction_pct'] = reduction_pct
            features['orig_volume_m3'] = 1.0  # Default value
        
        # Pitch (if available)
        if 'pitch_m' in part_data and pd.notna(part_data['pitch_m']):
            features['pitch_m'] = part_data['pitch_m']
        else:
            features['pitch_m'] = 0.001  # Default 1mm pitch
        
        # Density (if available)
        if 'density_kg_m3' in part_data and pd.notna(part_data['density_kg_m3']):
            features['density_kg_m3'] = part_data['density_kg_m3']
        else:
            features['density_kg_m3'] = 2700  # Default aluminum density
...
def main():
    ...
    print("=== THINNING RECOMMENDATION SYSTEM ===")
    ...
    if recommendations:
        output_path = "thinning_recommendations.csv"
        df_rec = pd.DataFrame(recommendations)
